...
from .data_preprocessing import pad_sequence, onehot_encode_sequence

logger = logging.getLogger(__name__)


class LSTMNextActivityPredictorOneHot:
    """
    LSTM-based next activity predictor with one-hot encoding.
    ...
    def __init__(
        self,
        model_path: str = "models/next_activity_lstm_onehot",
        ...
        self.vocab_size = metadata.get('vocab_size', len(metadata.get('activity_to_idx', {})))
        ...
        if self.vocab_size == 0:
            self.vocab_size = len(self.activity_to_idx)
        ...
        logger.info(f"Loaded LSTMNextActivityPredictorOneHot from {model_path}")
        logger.info(f"Vocabulary size: {self.vocab_size} (including END)")
    ...
    def predict(self, case_state) -> tuple[str, bool]:
        ...
        try:
            ...
            sequence_indices = np.array([sequence_padded], dtype=np.int32)
            
            sequence_onehot = onehot_encode_sequence(sequence_indices[0], self.vocab_size)
