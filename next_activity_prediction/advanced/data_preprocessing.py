"""
Data preprocessing for next activity prediction.
...
def prepare_training_data(
    ...
    - Input: [a1, a2, ..., a_i] (padded/truncated)
    ...
    Returns:
        Tuple of (X, y_activity, position_info) numpy arrays
    ...
    for seq in sequences:
        ...
        seq_length = len(seq) - 1  # Excluding END token
        # Each sequence ends with END, so we iterate up to len(seq)
        for i in range(1, len(seq)):
            input_seq = seq[:i]
            next_activity = seq[i]  # This can be a regular activity or END
            ...
            if return_position_info:
                # Relative position in sequence (0.0 = start, 1.0 = end)
    ...
    if return_position_info and positions:
        ...
        return X, y_activity, positions
    
    return X, y_activity, None
...
def preprocess_event_log(
    ...
    """
    Complete preprocessing pipeline from event log to training data.
